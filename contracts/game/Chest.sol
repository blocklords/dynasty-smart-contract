...
contract Chest is IERC721Receiver, Pausable, Ownable {
    ...
    event ChestOpened(address indexed player, uint256[] nftTypeIndices, uint256[] itemCodes, uint256[] receiptIds, uint256[] tokenIds, uint256 time);       // Event emitted when a chest is opened
    ...
    function openChest(uint256 _seasonId, bytes calldata _data, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external nonReentrant whenNotPaused {
        ...
        emit ChestOpened(msg.sender, nftTypeIndices, itemCodes, receiptIds, tokenIds, block.timestamp);
    }
